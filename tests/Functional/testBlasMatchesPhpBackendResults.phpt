--TEST--
The blas backend produces the same product as the pure PHP backend
--INI--
ffi.enable=1
opcache.jit=off
...
// precision, so the two drivers must agree bit for bit despite summing the products in a different order
$size  = 8;
$left  = [];
$right = [];
